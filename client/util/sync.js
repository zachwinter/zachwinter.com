...
import { scaleLog, scaleLinear, scalePow, scaleSqrt } from 'd3-scale'
...
export default class Sync {
  constructor ({
    ...
    pingDelay = 2500,
    fixed = false,
    staticIntervalBaseDuration = 1000,
  ...
  buildStaticIntervals (base) {
    ...
    types.forEach(type => {
      ...
      for (var i = 0; i < 10000; i++) {
        ...
        analysis[type].push({
          start: i * duration[type],
          duration: duration[type],
          loudness_start: -20 + (i/100),
          loudness_max: -20 + (i/100),
