...
class Visualizer extends SpotifyConnect {
  constructor() {
    ...
    if ( window.location.hash && window.location.hash.indexOf('#offset=') > -1 ) {
      this.manualOffset = parseInt( window.location.hash.replace('#offset=', ''), 10 )
    }

    if ( window.location.hash && window.location.hash.indexOf('#outline=') > -1 ) {
      this.manualOffset = parseInt( window.location.hash.replace('#outline=', ''), 10 )
    }
  }
  ...
  processResponse(response) {
    ...
    console.log(`Sync error: ${parseInt(syncError - this.manualOffset, 10) || '0'}ms`)

    const getData = (noToast, noSyncToast) => {
      if (!noSyncToast ) {
        this.toast.syncing()
      }
      ...
      Promise.all([
        ...
        this.updateTrackProgress((response.delay) + (window.performance.now() - timestamp) - this.manualOffset)
        this.startVisualizer(noToast)
        this.pingSpotify()
      })
    }

    if (this.active && response.is_playing && songsInSync && (syncError - this.manualOffset) > 500) {
