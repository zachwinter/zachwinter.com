...
class Kaleidoscope extends Visualizer {
  ...
  setRadiusState() {
    ...
    for (var i = 0; i < this.totalStars; i++) {
      size = parseInt(size - this.sizeStep.randomElement(), 10)
  ...
  tweenRGB(duration, next, last, element, setColor) {
    let stepR = (parseInt(next[0], 10) - parseInt(last[0], 10)) / (duration / this.refreshRate)
    let stepG = (parseInt(next[1], 10) - parseInt(last[1], 10)) / (duration / this.refreshRate)
    let stepB = (parseInt(next[2], 10) - parseInt(last[2], 10)) / (duration / this.refreshRate)
    
    let tweenR = parseInt(last[0], 10)
    let tweenG = parseInt(last[1], 10)
    let tweenB = parseInt(last[2], 10)

    element.colorTween = setInterval(() => {
      tweenR = parseInt(tweenR + stepR, 10)
      tweenG = parseInt(tweenG + stepG, 10)
      tweenB = parseInt(tweenB + stepB, 10)
